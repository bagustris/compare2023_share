...
def create_split(partition:str, out_dir:Path, data_info=None, type_info=None):
    ...
    # print len(data) and len(whole_data) to check if there are any missing files
    assert len(data) == len(whole_data)
    # print("There are {} audio files".format(len(type_data)))
    ...
    # print 10 first data
    # print(part_data.head(10))
    print(f"Number of {str(partition)}: {len(part_data)}")
