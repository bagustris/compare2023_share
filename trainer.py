...
class Trainer():
    ...
    def __init__(self, params: Params) -> None:
        ...
        # create root directory if they don't exist
        ...
        # self.mse_loss = MSE_loss(params=params)
        # if running on GPU, move criterion there, maybe source of error
        if params.cuda:
            self.criterion.cuda()
    ...
    def train(self):
        ...
        for epoch in range(1, epochs + 1):
            ...
            for index, batch in enumerate(self.train_loader):
                ...
                loss, task_losses = self.criterion.forward(
                        out, labels, return_all=True)  # process these dicts
                ...
                if index % self.params.train.save_summary_steps == 0:
                    self.writer.add_scalar(
                        "train/loss",
                        loss.item(),
                        # global index across epochs
                        global_step=(epoch - 1) * \
                            len(self.train_loader) + index
                    )
