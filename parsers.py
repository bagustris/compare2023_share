...
class Options():
    ...
    def _add_train_args(self, parser:ArgumentParser) -> ArgumentParser:
        ...
        """
        # dataset
        parser.add_argument("--num_train_workers", type=int, default=10,
                            help="Number of workers for loading train data (defaults to 10")
        ...
        # TODO: find optimum values of weight decay in [1e-4, 1e-3]
        parser.add_argument("--weight_decay", type=float, default=0.001, help="Weight decay")
        # parser.add_argument("--loss", type=str, default="ccc", choices=["ccc", "mse"], help="Loss to use for the regression tasks. Default is CCC loss")
        parser.add_argument("--loss_strategy", type=str, default="ccc", choices=["ccc", "mse"], help="Loss function to use")
    ...
    def _add_eval_args(self, parser:ArgumentParser) -> ArgumentParser:
        ...
        parser.add_argument("--num_val_workers", type=int, default=10,
                            help="Number of workers for loading eval data (defaults to 10")
        parser.add_argument("--val_dataset_file", type=str, required=True, help="Path to the csv file that stores the validation labels")
        # test 
        parser.add_argument("--num_test_workers", type=int, default=10, help="Number of workers for loading test data (default 10)")
        parser.add_argument("--test_dataset_file", type=str, required=True, help="Path to the csv file that holds test file info")
       
        parser.add_argument("--metrics", type=str, help="Metric to use for validation", default="ccc", choices=["pearson, spearmann"])  # TODO maybe do this via config file per task?
        parser.add_argument("--prediction_file", type=str, default="predictions.csv", help="The file to write test predictions in csv format")
    ...
    def _add_parsers(self) -> argparse.ArgumentParser():
        ...
        parser.add_argument("-n", "--name", type=str, default="experiment_2", help="Experiment ID")
        # MODEL
        # vanilla=basemtl, chain=stacked, branching=attnbranch, fusion=attnfusion
        parser.add_argument("--model_name", type=str, default="stacked", choices=["basemtl", "stacked", "nn", "attnfusion", "attnbranch", "attnbranch_v2"], help="Name of model architecture to use")
        parser.add_argument("--feature_extractor", type=str, default="hubert-large-ll60k", help="Feature network architecture to use. See the list in the readme")
        parser.add_argument("--num_outputs", type=int, default=1, help="Number of model outputs (default 1)")
        parser.add_argument("--num_tasks", type=int, default=1, help="Number of tasks to train/predict for. Default is 1")
        ...
        # TODO: find optimal values for spec augment mask prob in [0.05, 0.1]
        parser.add_argument("--mask_feature_prob", type=float, default=0.05, help="SpecAugment feature mask probability (default 0.0)")
        ...
        # Path
        parser.add_argument("--root_dir", type=str, default="./result", help="Root folder for the output")
