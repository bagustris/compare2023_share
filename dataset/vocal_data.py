...
class VocalDataset(Dataset):
    ...
    def __init__(self, params:Params) -> None:
        ...
        self.meta = pd.read_csv(str(self.dataset_file), header="infer", dtype={"filename": "str"})
    ...
    def __getitem__(self, index):
        ...
        wav_id = self.meta.loc[index, "filename"]
        out["fid"] = wav_id
        # load audio
        ...
        if str(self.params.partition).lower() != "test":
            ...
            # out["low"] = low
            # high
            # high = self.meta.iloc[index, 5:15].to_numpy("float32")   
            high = self.meta.iloc[index, 1:].to_numpy("float32")
            out["high"] = high
    ...
    def load_wav(self, id):
        ...
        wav, sr = torchaudio.load(str(self.wav_folder / "{}".format(id)))
