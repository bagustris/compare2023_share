...
def CCC(preds:torch.Tensor, targets:torch.Tensor):
    ...
    ccc = 2 * cov / (preds_var + targets_var + torch.square(
        preds_mean - targets_mean)) # [N]
...
def pearson_correlation_coefficient(preds:np.ndarray, targets:np.ndarray):
    ...
    """
    # print(f"type of preds: {type(preds)}, shape of preds: {preds.shape}")
    ...
    # print(f"pearson type: {type(pearson)}, shape: {pearson.shape}")
    return pearson


def spearmann_correlation_coefficient(preds: np.ndarray, targets: np.ndarray):
    """
    compute spearmann correlation coefficient using scipy.stats.spearmanr
    """
    spearmann, pval = stats.spearmanr(preds, targets, axis=0)
    return spearmann
...
def UAR(preds: torch.Tensor, targets: torch.Tensor):
    ...
    return recall_score(y_true=targets.numpy(), y_pred=preds.numpy(), 
                        waverage="macro")
...
class Metric():
    ...
    def __init__(self, params: Params = None) -> None:
        ...
        self.regression_metrics = {
            # "Spearmann": spearmann_correlation_coefficient, # still error
